...
function renderUserList(data) {
    ...
    data.forEach((item) => {
        ...
        //                         <i class="fa fa-times btn-add-favorite" aria-hidden="true" data-id="${item.id}"></i>
        //                         </button>
        //                 </div>
        //             </div>
        ...
        // `;
        rawHTML += `
<div class="card m-2 card-box text-center">
                        ...
                        />
                        <div class='m-3 d-flex justify-content-end'>
                                <button class="btn btn-outline-danger btn-add-favorite"data-id="${item.id}" >
                                  <i class="fa fa-times " 
                                      aria-hidden="true" >
                                  </i>
                                </button>
...
function removeFromBestFriend(id) {
    function isUserIdMatched(user) {
        return user.id===id
    }
    const userIndex = users.findIndex(isUserIdMatched)
...
dataPanel.addEventListener('click', function onPanelClicked(e) {
    if (e.target.matches('.show-user')) {
        ...
        removeFromBestFriend(Number(e.target.dataset.id))
    }
});
